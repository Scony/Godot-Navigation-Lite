...
class DetourNavigationMeshCacheGenerator : public DetourNavigationMeshGenerator
{
 ...
 public:
  ...
  unsigned int build_tiles(int x1, int z1, int x2, int z2);
  ...
  void set_tile_cache(dtTileCache* tc) { tile_cache = tc; }

  void set_mesh_process(NavMeshProcess* nmp) { mesh_process = nmp; }

  dtTileCache* get_tile_cache() { return tile_cache; }

  NavMeshProcess* get_mesh_process() { return mesh_process; }

  dtTileCacheCompressor* get_tile_cache_compressor() { return tile_cache_compressor; }
};
