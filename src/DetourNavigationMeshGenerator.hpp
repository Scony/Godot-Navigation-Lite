...
class DetourNavigationMeshGenerator
{
 public:
  ...
  {
    num_tiles_x =
        (xSize + navmesh_parameters->get_tile_size() - 1) / navmesh_parameters->get_tile_size();
    num_tiles_z =
        (zSize + navmesh_parameters->get_tile_size() - 1) / navmesh_parameters->get_tile_size();
  };
  ...
  void add_meshdata(int mesh_index, std::vector<float>& p_verticies, std::vector<int>& p_indices);
