...
void DetourNavigationMeshGenerator::build()
{
  ...
  unsigned int result = build_tiles(0, 0, get_num_tiles_x() - 1, get_num_tiles_z() - 1);
}
...
void DetourNavigationMeshGenerator::setup_generator()
{
  ...
  int gridH = 0, gridW = 0;
  rcCalcGridSize(
      &bmin.coord[0], &bmax.coord[0], navmesh_parameters->get_cell_size(), &gridW, &gridH);
...
void DetourNavigationMeshGenerator::joint_build()
{
  ...
  {
    bounding_box.merge_with(input_transforms->at(i).xform(input_aabbs->at(i)));
  }
  setup_generator();
  ...
  unsigned int tile_bits = (unsigned int)ilog2(
      nextPow2(get_num_tiles_x() * get_num_tiles_z() * navmesh_parameters->get_max_layers()));
...
unsigned int DetourNavigationMeshGenerator::build_tiles(int x1, int z1, int x2, int z2)
{
  unsigned ret = 0;
  for (int z = z1; z <= z2; z++)
  {
...
void DetourNavigationMeshGenerator::init_rc_config(rcConfig& config, Vector3& bmin, Vector3& bmax)
{
  ...
  config.detailSampleDist = navmesh_parameters->get_detail_sample_distance() < 0.9f
      ? 0.0f
      : config.cs * navmesh_parameters->get_detail_sample_distance();
    ...
    std::vector<int>& indices)
{
  ...
  if (!rcBuildCompactHeightfield(
          ctx, config.walkableHeight, config.walkableClimb, *heightfield, *compact_heightfield))
  {
...
bool DetourNavigationMeshGenerator::build_tile(int x, int z)
{
  ...
  if (dtStatusFailed(
          get_detour_navmesh()->addTile(nav_data, nav_data_size, DT_TILE_FREE_DATA, 0, NULL)))
  {
    dtFree(nav_data);
...
void DetourNavigationMeshGenerator::get_tile_bounding_box(
    ...
    Vector3& bmax)
{
  const float tile_edge_length =
      (float)navmesh_parameters->get_tile_size() * navmesh_parameters->get_cell_size();
  bmin =
      bounding_box.position + Vector3(tile_edge_length * (float)x, 0, tile_edge_length * (float)z);
...
void DetourNavigationMeshGenerator::mark_dirty(int start_index, int end_index)
{
  ...
  for (int aabb_index = start_index; aabb_index < end_index; aabb_index++)
  {
    ...
    Vector3 max =
        (changes_bounding_box.position + changes_bounding_box.size - (bounding_box.position)) /
        tile_edge_length;
    ...
    for (int i = std::max(0, int(std::floor(min.x)));
         i < std::min(int(std::ceil(max.x)), get_num_tiles_x());
         i++)
    {
      for (int j = std::max(0, int(std::floor(min.z)));
           j < std::min(get_num_tiles_z(), int(std::ceil(max.z)));
           j++)
      {
        dirty_tiles[i][j] = 1;
      }
