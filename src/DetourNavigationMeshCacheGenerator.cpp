...
void DetourNavigationMeshCacheGenerator::build()
{
  ...
  tile_cache_params.maxTiles =
      get_num_tiles_x() * get_num_tiles_z() * navmesh_parameters->get_max_layers() * 10;
...
unsigned int DetourNavigationMeshCacheGenerator::build_tiles(int x1, int z1, int x2, int z2)
{
  unsigned ret = 0;
  for (int z = z1; z <= z2; z++)
  {
...
bool DetourNavigationMeshCacheGenerator::build_tile(int x, int z)
{
  ...
  for (int i = 0; i < heightfield_layer_set->nlayers; i++)
  {
    ...
    int status =
        tile_cache->addTile(tile_data, tile_data_size, DT_COMPRESSEDTILE_FREE_DATA, &tileRef);
...
{
  if (dtStatusFailed(
          tile_cache->init(params, tile_cache_alloc, tile_cache_compressor, mesh_process)))
  {
