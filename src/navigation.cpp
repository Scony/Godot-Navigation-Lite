...
void DetourNavigation::_register_methods()
{
  ...
  register_method("add_cached_collision_shape", &DetourNavigation::_on_cache_collision_shape_added);
  register_method(
      "remove_cached_collision_shape", &DetourNavigation::_on_cache_collision_shape_removed);
  register_method("add_collision_shape", &DetourNavigation::_on_collision_shape_added);
  register_method("remove_collision_shape", &DetourNavigation::_on_collision_shape_removed);
  register_method("_on_tree_exiting", &DetourNavigation::_on_tree_exiting);

  register_property<DetourNavigation, bool>(
      "auto_add_remove_objects",
      &DetourNavigation::set_auto_object_management,
      &DetourNavigation::get_auto_object_management,
      true);
...
void DetourNavigation::_on_tree_exiting()
{
  if (!Engine::get_singleton()->is_editor_hint() && auto_object_management)
  {
    ...
    get_tree()->disconnect("node_removed", this, "remove_cached_collision_shape");
...
void DetourNavigation::fill_pointer_arrays()
{
  ...
  {
    DetourNavigationMeshCached* navmesh_pc =
        Object::cast_to<DetourNavigationMeshCached>(get_child(i));
...
void DetourNavigation::recalculate_masks()
{
  ...
  for (int i = 0; i < cached_navmeshes.size(); ++i)
  {
    ...
    dynamic_collision_mask |= cached_navmeshes[i]->get_dynamic_collision_mask();
  }
  for (int i = 0; i < navmeshes.size(); ++i)
  {
...
void DetourNavigation::add_box_obstacle_to_all(
    int64_t instance_id,
    Vector3 position,
    Vector3 extents,
    float rotationY,
    int collision_layer)
{
  for (int i = 0; i < cached_navmeshes.size(); ++i)
  {
    if (cached_navmeshes[i] != nullptr &&
        cached_navmeshes[i]->get_dynamic_collision_mask() & collision_layer)
    {
      unsigned int ref_id = cached_navmeshes[i]->add_box_obstacle(position, extents, rotationY);
...
void DetourNavigation::add_cylinder_obstacle_to_all(
    int64_t instance_id,
    Vector3 position,
    float radius,
    float height,
    int collision_layer)
{
  for (int i = 0; i < cached_navmeshes.size(); ++i)
  {
    if (cached_navmeshes[i] != nullptr &&
        cached_navmeshes[i]->get_dynamic_collision_mask() & collision_layer)
    {
      unsigned int ref_id = cached_navmeshes[i]->add_cylinder_obstacle(position, radius, height);
      cached_navmeshes[i]->dynamic_obstacles[instance_id] = (Variant)ref_id;
      cached_navmeshes[i]->debug_navmesh_dirty = true;
    }
  }
...
void DetourNavigation::manage_changes()
{
  ...
  if (collisions_to_remove.size() > 0)
  {
    for (int i = 0; i < navmeshes.size(); ++i)
    {
      ...
      {
        navmeshes[i]->generator->remove_collision_shape(collision_shape_id);
      }
      navmeshes[i]->generator->recalculate_tiles();
      navmeshes[i]->debug_navmesh_dirty = true;
    }
    for (int i = 0; i < cached_navmeshes.size(); ++i)
    {
      for (int64_t collision_shape_id : collisions_to_remove)
      {
        cached_navmeshes[i]->generator->remove_collision_shape(collision_shape_id);
      }
      cached_navmeshes[i]->recalculate_tiles();
      cached_navmeshes[i]->debug_navmesh_dirty = true;
    }
    ...
    for (PhysicsBody* physics_body : dyn_bodies_to_add)
    {
      for (int i = 0; i < physics_body->get_child_count(); ++i)
      {
        CollisionShape* collision_shape =
            Object::cast_to<CollisionShape>(physics_body->get_child(i));
        ...
        else if (s->get_class() == "CylinderShape")
        {
          ...
          add_cylinder_obstacle_to_all(
              ...
              cylinder->get_radius() *
                  std::max(
                      transform.get_basis().get_scale().x, transform.get_basis().get_scale().z),
...
void DetourNavigation::_on_cache_collision_shape_added(Variant node)
{
  ...
  if (collision_shape)
  {
    PhysicsBody* physics_body = Object::cast_to<PhysicsBody>(collision_shape->get_parent());
...
void DetourNavigation::build_navmesh_cached(DetourNavigationMeshCached* navmesh)
{
  ...
  DetourNavigationMeshCacheGenerator* dtnavmesh_gen =
      navmesh->init_generator(get_global_transform());
    ...
    std::vector<int64_t>* collision_ids)
{
  ...
  for (int i = 0; i < array_mesh->get_surface_count(); i++)
  {
    ...
    std::vector<std::vector<std::list<Vector3>>> mesh_instance_array(
        x_size, std::vector<std::list<Vector3>>(z_size));
    ...
    std::vector<int64_t>* collision_ids)
{
  ...
  else if (s->get_class() == "CylinderShape")
  {
    ...
    array_mesh->add_surface_from_arrays(
        Mesh::PRIMITIVE_TRIANGLES, cylinder_mesh->get_mesh_arrays());
    cylinder_mesh.unref();
    mesh = array_mesh;
  }
  ...
  if (mesh.is_valid())
  {
    ...
    if (!process_large_mesh(
            mi, collision_shape->get_instance_id(), meshes, transforms, aabbs, collision_ids))
    {
 ...
 */
void DetourNavigation::collect_geometry(
    Array geometries,
    std::vector<Ref<Mesh>>* meshes,
    std::vector<Transform>* transforms,
    std::vector<AABB>* aabbs,
  ...
  for (int i = 0; i < geom_size; i++)
  {
    ...
    else
    {
      ...
      if (static_body && static_body->get_collision_layer() & navmesh->get_collision_mask())
      {
        ...
        {
          CollisionShape* collision_shape =
              Object::cast_to<CollisionShape>(static_body->get_child(i));
          if (collision_shape)
          {
    ...
    if (spatial)
    {
      collect_geometry(spatial->get_children(), meshes, transforms, aabbs, collision_ids, navmesh);
    }
  }
}
  ...
  for (int i = 0; i < elements.size(); i++)
  {
    ...
    if (collision_shape)
    {
      ...
      std::string key_string =
          (std::to_string(gt.origin.x) + "x" + std::to_string(gt.origin.y) + "x" +
           std::to_string(gt.origin.z));
...
void DetourNavigation::map_collision_shapes(DetourNavigationMesh* nm, Dictionary& mappings)
{
  ...
  for (int j = 0; j < nm->generator->input_transforms->size(); ++j)
  {
    ...
    std::string key_string =
        (std::to_string(it.origin.x) + "x" + std::to_string(it.origin.y) + "x" +
         std::to_string(it.origin.z));
