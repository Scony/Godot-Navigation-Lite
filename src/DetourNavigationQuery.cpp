...
void DetourNavigationQuery::init(dtNavMesh* dtMesh, const Transform& xform)
{
    ...
    DetourNavigationQueryFilter* filter)
{
  ...
  status = navmesh_query->findNearestPoly(
      &start.coord[0], &extents.coord[0], filter->dt_query_filter, &StartPoly, StartNearest);
  if ((status & DT_FAILURE) || (status & DT_STATUS_DETAIL_MASK))
    return ret; // couldn't find a polygon

  // find the end polygon
  status = navmesh_query->findNearestPoly(
      &end.coord[0], &extents.coord[0], filter->dt_query_filter, &EndPoly, EndNearest);
  if ((status & DT_FAILURE) || (status & DT_STATUS_DETAIL_MASK))
    return ret; // couldn't find a polygon

  status = navmesh_query->findPath(
      StartPoly,
      EndPoly,
      StartNearest,
      EndNearest,
  ...
  status = navmesh_query->findStraightPath(
      StartNearest,
      EndNearest,
      PolyPath,
      nPathCount,
  ...
  {
    points_write[nVert] =
        Vector3(StraightPath[nIndex], StraightPath[nIndex + 1], StraightPath[nIndex + 2]);
    nIndex += 3;
  }
